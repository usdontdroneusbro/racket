...
@defproc*[([(in-range [end number?]) stream?]
  ...
  case @racket[(in-range end)] is equivalent to @racket[(in-range 0 end
  1)].  The first number in the sequence is @racket[start], and each
  successive element is generated by adding @racket[step] to the
...
@defproc[(in-lines [in input-port? (current-input-port)]
  ...
  @racket[(in-port (lambda (p) (read-line p mode)) in)].  Note that the
  default mode is @racket['any], whereas the default mode of
...
@defproc[(in-hash-pairs [hash hash?]) sequence?]{
  ...
  its value from @racket[hash] (as opposed to using @racket[hash]
...
@defproc[(in-producer [producer procedure?] [stop any/c] [args any/c] ...)
  ...
  with @racket[eq?].  (The @racket[stop] argument must be a predicate
...
@defproc[(in-value [v any/c]) sequence?]{
  ...
  form is mostly useful for @racket[let]-like bindings in forms such as
  @racket[for*/list].}
...
@defproc[(in-parallel [seq sequence?] ...) sequence?]{
  ...
  each @racket[seq].  The elements of each @racket[seq] must be
  single-valued.}
...
@defproc[(make-do-sequence [thunk (-> (values (any/c . -> . any)
  ...
  @itemize[
    @item{The first result is a @racket[_pos->element] procedure that takes
      the current position and returns the value(s) for the current
      element.}
    @item{The second result is a @racket[_next-pos] procedure that takes
  ...
  returns @racket[#f], the sequence ends, and none are called again.
  Typically, one of the functions determines the end condition, and
  @racket[#f] is used in place of the other two functions.}
...
@defproc[(sequence-generate [seq sequence?])
  ...
  returns @racket[#t] if more values are available for the sequence.
...
@defproc[(sequence->list [s sequence?]) list?]{
  Returns a list whose elements are the elements of @racket[s],
  each of which must be a single value.  If @racket[s] is infinite, this
...
@defproc[(sequence-map [f procedure?]
                       [s sequence?])
         sequence?]{
  Returns a sequence that contains @racket[f] applied to each element of
  @racket[s].  The new sequence is constructed lazily.
...
@defproc[(sequence-andmap [f (-> any/c ... boolean?)]
  ...
  element of @racket[s].  If @racket[s] is infinite and @racket[f] never
...
@defproc[(sequence-count [f procedure?] [s sequence?])
  ...
  returns a true result.  If @racket[s] is infinite, this function does
  not terminate.}
...
@defproc[(sequence-filter [f (-> any/c ... boolean?)]
  ...
  which @racket[f] returns a true result.  Although the new sequence is
  constructed lazily, if @racket[s] has an infinite number of elements
  where @racket[f] returns a false result in between two elements where
  @racket[f] returns a true result, then operations on this sequence will
...
@defproc[(stream-filter [f (-> any/c ... boolean?)]
  ...
  constructed lazily, if @racket[s] has an infinite number of elements
  where @racket[f] returns a false result in between two elements where
  @racket[f] returns a true result, then operations on this stream will
  not terminate during the infinite sub-stream.}

@defproc[(stream-add-between [s stream?] [e any/c])
         stream?]{
  Returns a stream whose elements are the elements of @racket[s],
  but with @racket[e] between each pair of elements in @racket[s].
...
@defform[(infinite-generator body ...+)]{
  Like @racket[generator], but repeats evaluation of the @racket[body]s when
...
@defproc[(sequence->repeated-generator [s sequence?]) (-> any)]{
  Like @racket[sequence->generator], but when @racket[s] has no further
